...
Public useHiddenShare As Boolean
...
Private Sub mnuFileSaveAs_Click()
    ...
    sFile = getFileFromHiddenShare("Save As", "*.smmd;*.txt")
...
Private Sub mnuFileSave_Click()
    ...
    If Left$(ActiveForm.Caption, 8) = "Document" Then
        sFile = getFileFromHiddenShare("Save", "*.smmd;*.txt")
        ActiveForm.rtfText.SaveFile sFile
    Else
...
Private Sub mnuFileOpen_Click()
    ...
    sFile = getFileFromHiddenShare("Open", "*.smmd;*.txt")
...
'   useHiddenShare is currently a global in this test file which would be set
'   at launch to when it is determined whether or not a hidden share exists.
'   If there is no hidden share, the current dlgCommonDialog is used, as before
'   This example does not have any nuances which may currently be in Fusion AE.

'   sDialogTitle will be displayed in the title bar of the window
'   sFilter is the filter applied to files listed in the dialog.

'   The hidden share path is hard coded in this method.

Private Function getFileFromHiddenShare(sDialogTitle, sFilter) As String
    useHiddenShare = True
    If useHiddenShare = True Then
     
        With frmHiddenShareDialog
            .Caption = sDialogTitle
            .FusionAEPath.Path = "\\VIVALDI2GO\FusionAEData$"
            .FusionAEFile.Pattern = sFilter
            .Show vbModal, Me
            If Len(.FusionFileName.Text) = 0 Then
                Exit Function
            End If
            getFileFromHiddenShare = .FusionFileName.Text
        End With
    ...
    Else
        ...
        With dlgCommonDialog
            ...
            .Filter = "Fusion AE Files (" & sFilter & ")|" & sFilter
            
            .ShowOpen
            If Len(.FileName) = 0 Then
                Exit Function
            End If
            getFileFromHiddenShare = .FileName
        End With
    End If
