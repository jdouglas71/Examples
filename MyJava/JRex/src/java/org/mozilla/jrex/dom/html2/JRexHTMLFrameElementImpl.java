...
* @version 1.0
* @see org.w3c.dom.html2.HTMLFrameElement
*/
public class JRexHTMLFrameElementImpl extends JRexHTMLElementImpl implements HTMLFrameElement {
	private static final String className	= JRexHTMLFrameElementImpl.class.getName();

	/** Intended to be used by JNI */
    private JRexHTMLFrameElementImpl() {
		if(JRexL.on)JRexL.log(className,"**** JRexHTMLFrameElementImpl ****");
	}
	...
	public boolean getNoResize(){
		if(JRexL.on)JRexL.log(className,"**** getNoResize I AM HERE ****");
		boolean noResize=false;
		try{
			noResize=GetNoResize();
		}catch(JRexException ex){
			JRexL.printStackTrace(ex);
		}
		if(JRexL.on)JRexL.log(className,"**** getNoResize noResize<"+noResize+"> ****");
		return noResize;
	}

	public void setNoResize(boolean noResize){
		if(JRexL.on)JRexL.log(className,"**** setNoResize noResize<"+noResize+"> ****");
		try{
    ...
    private native boolean GetNoResize()throws JRexException;
    private native void SetNoResize(boolean noResize)throws JRexException;
