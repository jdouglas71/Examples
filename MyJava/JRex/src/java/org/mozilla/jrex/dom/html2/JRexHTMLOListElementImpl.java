...
* @version 1.0
* @see org.w3c.dom.html2.HTMLOListElement
*/

public class JRexHTMLOListElementImpl extends JRexHTMLElementImpl implements HTMLOListElement {

	private static final String className	= JRexHTMLOListElementImpl.class.getName();

     /** Intended to be used by JNI */
    private JRexHTMLOListElementImpl() {
		if(JRexL.on)JRexL.log(className,"**** JRexHTMLOListElementImpl ****");
	}
	...
	public int getStart(){
		if(JRexL.on)JRexL.log(className,"**** getStart I AM HERE ****");
		int start=0;
		try{
			start=GetStart();
		}catch(JRexException ex){
		...
		if(JRexL.on)JRexL.log(className,"**** getStart start<"+start+"> ****");
		return start;
	}

	public void setStart(int start){
		if(JRexL.on)JRexL.log(className,"**** setStart start<"+start+"> ****");
		try{
			SetStart(start);
		}catch(JRexException ex){
			JRexL.printStackTrace(ex);
	...
	private native int GetStart()throws JRexException;
	private native void SetStart(int start)throws JRexException;
