...
/**
* The JRex Event handler for progress events.
* Intended to be used by JRex EventManager.
* @author <a href="mailto:jrex_moz@yahoo.com">C.N.Medappa</a>
* @version 1.0
*/
public class ProgressEventHandler implements JRexEventHandler{

	private static final String className = ProgressEventHandler.class.getName();
	...
	public Object handleEvent(JRexEvent event){
		...
		ProgressEvent pe=(ProgressEvent)event.getData();
		if(JRexL.on)JRexL.log(className,"**** handleEvent()-->pe <"+pe+"> ****");
		assert pe!=null;
		if(pe==null)return null;
		if(windowManager==null)
			windowManager=(JRexWindowManager)JRexFactory.getInstance().getImplInstance(JRexFactory.WINDOW_MANAGER);
		JRexCanvas browser=windowManager.getBrowser(pe.getJRexPeer());
		assert browser!=null;
		if(browser==null)return null;
		browser.processProgressEvent(pe);
		return null;
	}
}