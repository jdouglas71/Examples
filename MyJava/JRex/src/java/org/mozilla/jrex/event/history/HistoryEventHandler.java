...
/**
* The JRex Event handler for History events.
* Intended to be used by JRex EventManager.
* @author <a href="mailto:jrex_moz@yahoo.com">C.N.Medappa</a>
* @version 1.0
*/
public class HistoryEventHandler implements JRexEventHandler{

	private static final String className 				= HistoryEventHandler.class.getName();
	...
	public Object handleEvent(JRexEvent event){
		...
		HistoryEvent he=(HistoryEvent)event.getData();
		if(JRexL.on)JRexL.log(className,"**** handleEvent()-->he <"+he+"> ****");
		assert he!=null;
		if(he==null)return null;
		if(windowManager==null)
			windowManager=(JRexWindowManager)JRexFactory.getInstance().getImplInstance(JRexFactory.WINDOW_MANAGER);
		JRexCanvas browser=windowManager.getBrowser(he.getJRexPeer());
		assert browser!=null;
		if(browser==null)return null;
		return browser.processHistoryEvent(he);
	}
}