...
/**
* The JRex Event handler for tooltip events.
* Intended to be used by JRex EventManager.
* @author <a href="mailto:jrex_moz@yahoo.com">C.N.Medappa</a>
* @version 1.0
*/
public class TooltipEventHandler implements JRexEventHandler{

	private static final String className = TooltipEventHandler.class.getName();
	...
	public Object handleEvent(JRexEvent event){
		...
		TooltipEvent tte=(TooltipEvent)event.getData();
		if(JRexL.on)JRexL.log(className,"**** handleEvent()-->tte <"+tte+"> ****");
		assert tte!=null;
		if(tte==null)return null;
		if(windowManager==null)
			windowManager=(JRexWindowManager)JRexFactory.getInstance().getImplInstance(JRexFactory.WINDOW_MANAGER);
		JRexCanvas browser=windowManager.getBrowser(tte.getJRexPeer());
		assert browser!=null;
		if(browser==null)return null;
		browser.processTooltipEvent(tte);
		if(JRexL.on)JRexL.log(className,"**** Tooltip handleEvent done ****");
		return null;
	}
}