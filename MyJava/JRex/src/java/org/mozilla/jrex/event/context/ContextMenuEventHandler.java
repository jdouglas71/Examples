...
/**
* The JRex Event handler for ContextMenu events.
* Intended to be used by JRex EventManager.
* @author <a href="mailto:jrex_moz@yahoo.com">C.N.Medappa</a>
* @version 1.0
*/
public class ContextMenuEventHandler implements JRexEventHandler{

	private static final String className 	= ContextMenuEventHandler.class.getName();
	...
	public Object handleEvent(JRexEvent event){
		...
		ContextMenuEvent ce=(ContextMenuEvent)event.getData();
		if(JRexL.on)JRexL.log(className,"**** handleEvent()-->ce <"+ce+"> ****");
		assert ce!=null;
		if(ce==null)return null;
		if(windowManager==null)
			windowManager=(JRexWindowManager)JRexFactory.getInstance().getImplInstance(JRexFactory.WINDOW_MANAGER);
		JRexCanvas browser=windowManager.getBrowser(ce.getJRexPeer());
		assert browser!=null;
		if(browser==null)return null;
		browser.processContextMenuEvent(ce);
		if(JRexL.on)JRexL.log(className,"**** ContextMenu handleEvent done ****");
		return null;
	}
}